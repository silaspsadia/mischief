...
echo_write(struct cdev *dev, struct uio *uio, int ioflag)
{
	...
	amount = MIN(uio->uio_resid,
		(echo_message->bufsize - 1 - uio->uio_offset > 0) ?
		echo_message->bufsize - 1 - uio->uio_offset : 0);
...
static int
echo_set_bufsize(int size)
{
	int error = 0;
	
	if (echo_message->bufsize == size)
		return error;

	if (size >= 128 && size <= 512) {
		echo_message->buffer = realloc(echo_message->buffer, size,
			M_ECHO, M_WAITOK);
		echo_message->bufsize = size;
...
static int 
sysctl_set_bufsize(SYSCTL_HANDLER_ARGS)
{
	int error = 0;
	int size = echo_message->bufsize;

	error = sysctl_handle_int(oidp, &size, 0, req);
	if (error || !req->newptr || echo_message->bufsize == size)
		return error;
...
echo_modevent(module_t mod __unused, int event, void *arg __unused)
{
	...
	case MOD_LOAD:
		...
		echo_message->bufsize = 256;
		echo_message->buffer = malloc(echo_message->bufsize,
			M_ECHO, M_WAITOK);
		sysctl_ctx_init(&clist);
		poid = (struct sysctl_oid *)SYSCTL_ADD_NODE(&clist, SYSCTL_STATIC_CHILDREN(/* tree top */), OID_AUTO, "echo", CTLFLAG_RW, 0, "echo root node");
		SYSCTL_ADD_PROC(&clist, SYSCTL_CHILDREN(poid), OID_AUTO, "bufsize", CTLTYPE_INT | CTLFLAG_RW, &echo_message->bufsize, 0, sysctl_set_bufsize, "I", "echo bufsize");
